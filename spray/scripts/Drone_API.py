...
from mavros_msgs.msg import GlobalPositionTarget, ParamValue, State, RCIn, OverrideRCIn
...
class MAVROS_Drone():
    ...
    def init_publishers(self):
        ...
        self.pwm_pub = rospy.Publisher(f'{self.ns}/mavros/rc/override', OverrideRCIn, queue_size=10)

    def set_pwm(self, channel, pwm_value):
        """
        Set a PWM value for a specific channel.

        :param channel: int, channel number (1-based index)
        :param pwm_value: int, PWM value in microseconds (1000-2000 range)
        """
        if not (1 <= channel <= 8):
            rospy.logerr("Channel must be between 1 and 8.")
            return

        pwm_msg = OverrideRCIn()
        pwm_msg.channels = [65535] * 8  # Default all channels to 65535 (ignored)
        pwm_msg.channels[channel - 1] = pwm_value  # Set the desired channel
        self.pwm_pub.publish(pwm_msg)
        rospy.loginfo(f"Set PWM on channel {channel} to {pwm_value}")
